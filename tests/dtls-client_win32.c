...
static int key_was_pressed(void)
{
  return _kbhit();
}
...
handle_stdin() {
  ...
  while (_kbhit())
  {
    ...
    if (key == 13)
    {
      buf[input_cnt++] = (char)key;
      len = input_cnt;
      input_cnt = 0;
      printf("\n");
    }
    else if (input_cnt < sizeof(buf))
    {
      buf[input_cnt++] = (char)key;
      printf("%c", key);
      fflush(stdout);
...
main(int argc, char **argv)
{
  ...
  res = resolve_address(argv[1], &dst.addr.sa);
  if (res < 0) {
  ...
  port, otherwise */
  dst.addr.sin.sin_port = htons(atoi(argv[2]));
  ...
  printf("Type text to send message to DTLS server, then server will echo that!\n");
